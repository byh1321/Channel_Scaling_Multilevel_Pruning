...
test_loader = torch.utils.data.DataLoader(cifar_test,batch_size=5000, shuffle=False,num_workers=8,drop_last=False)
...
class CNN(nn.Module):
	def __init__(self):
		...
		self.conv1 = nn.Sequential(
			nn.Conv2d(3,int(64*args.cs),3,padding=1,bias=False), #layer0
			nn.BatchNorm2d(int(64*args.cs)), # batch norm is added because dataset is changed
			nn.ReLU(inplace=True),
		)
		self.conv2 = nn.Sequential(
			nn.Conv2d(int(64*args.cs),int(64*args.cs),3,padding=1, bias=False), #layer3
			nn.BatchNorm2d(int(64*args.cs)),
			nn.ReLU(inplace=True),
		)
		...
		self.conv3 = nn.Sequential(
			nn.Conv2d(int(64*args.cs),int(128*args.cs),3,padding=1, bias=False), #layer7
			nn.BatchNorm2d(int(128*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv4 = nn.Sequential(
			nn.Conv2d(int(128*args.cs),int(128*args.cs),3,padding=1, bias=False),#layer10
			nn.BatchNorm2d(int(128*args.cs)),
			nn.ReLU(inplace=True),
		)
		...
		self.conv5 = nn.Sequential(
			nn.Conv2d(int(128*args.cs),int(256*args.cs),3,padding=1, bias=False), #layer14
			nn.BatchNorm2d(int(256*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv6 = nn.Sequential(
			nn.Conv2d(int(256*args.cs),int(256*args.cs),3,padding=1, bias=False), #layer17
			nn.BatchNorm2d(int(256*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv7 = nn.Sequential(
			nn.Conv2d(int(256*args.cs),int(256*args.cs),3,padding=1, bias=False), #layer20
			nn.BatchNorm2d(int(256*args.cs)),
			nn.ReLU(inplace=True),
		)
		...
		self.conv8 = nn.Sequential(
			nn.Conv2d(int(256*args.cs),int(512*args.cs),3,padding=1, bias=False), #layer24
			nn.BatchNorm2d(int(512*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv9 = nn.Sequential(
			nn.Conv2d(int(512*args.cs),int(512*args.cs),3,padding=1, bias=False), #layer27
			nn.BatchNorm2d(int(512*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv10 = nn.Sequential(
			nn.Conv2d(int(512*args.cs),int(512*args.cs),3,padding=1, bias=False), #layer30
			nn.BatchNorm2d(int(512*args.cs)),
			nn.ReLU(inplace=True),
		)
		...
		self.conv11 = nn.Sequential(
			nn.Conv2d(int(512*args.cs),int(512*args.cs),3,padding=1, bias=False), #layer34
			nn.BatchNorm2d(int(512*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv12 = nn.Sequential(
			nn.Conv2d(int(512*args.cs),int(512*args.cs),3,padding=1, bias=False), #layer37
			nn.BatchNorm2d(int(512*args.cs)),
			nn.ReLU(inplace=True),
		)
		self.conv13 = nn.Sequential(
			nn.Conv2d(int(512*args.cs),int(512*args.cs),3,padding=1, bias=False), #layer40
			nn.BatchNorm2d(int(512*args.cs)),
			nn.ReLU(inplace=True),
		)
		...
		self.fc1 = nn.Sequential(
			nn.Dropout(p=0.5),
			nn.Linear(int(512*args.cs),int(512*args.cs), bias=False), #fc_layer1
			nn.ReLU(inplace=True),
		)
		self.fc2 = nn.Sequential(
			nn.Dropout(p=0.5),
			nn.Linear(int(512*args.cs),int(512*args.cs), bias=False), #fc_layer4
			nn.ReLU(inplace=True),
		)
		self.fc3 = nn.Sequential(
			nn.Linear(int(512*args.cs),100, bias=False) #fc_layer6
		)
...
def maskgen():
	mask = []
	mask_conv0 = torch.zeros(int(64*args.cs),3,3,3).cuda()
	mask_conv1 = torch.zeros(int(64*args.cs),int(64*args.cs),3,3).cuda()
	mask_conv2 = torch.zeros(int(128*args.cs),int(64*args.cs),3,3).cuda()
	mask_conv3 = torch.zeros(int(128*args.cs),int(128*args.cs),3,3).cuda()
	mask_conv4 = torch.zeros(int(256*args.cs),int(128*args.cs),3,3).cuda()
	mask_conv5 = torch.zeros(int(256*args.cs),int(256*args.cs),3,3).cuda()
	mask_conv6 = torch.zeros(int(256*args.cs),int(256*args.cs),3,3).cuda()
	mask_conv7 = torch.zeros(int(512*args.cs),int(256*args.cs),3,3).cuda()
	mask_conv8 = torch.zeros(int(512*args.cs),int(512*args.cs),3,3).cuda()
	mask_conv9 = torch.zeros(int(512*args.cs),int(512*args.cs),3,3).cuda()
	mask_conv10 = torch.zeros(int(512*args.cs),int(512*args.cs),3,3).cuda()
	mask_conv11 = torch.zeros(int(512*args.cs),int(512*args.cs),3,3).cuda()
	mask_conv12 = torch.zeros(int(512*args.cs),int(512*args.cs),3,3).cuda()
	mask_fc0 = torch.zeros(int(512*args.cs),int(512*args.cs)).cuda()
	mask_fc1 = torch.zeros(int(512*args.cs),int(512*args.cs)).cuda()
	mask_fc2 = torch.zeros(100,int(512*args.cs)).cuda()
...
if args.mode == 0:
	...
	#checkpoint = torch.load('./checkpoint/ckpt_cs_'+str(args.cs*10000)+'_20190125.t0')
	net = checkpoint['net']
...
def test():
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			if args.mode == 1:
				torch.save(state, './checkpoint/ckpt_cs_'+str(args.cs*10000)+'_20190214.t0')
			if args.mode == 2:
				torch.save(state, './checkpoint/ckpt_cs_'+str(args.cs*10000)+'_20190214_pr_'+str(args.pr)+'.t0')
